...
N = int(4)  # mesh cells in x- and y-direction
...
for N in Nlist:
    ...
    def get_vertex_edge(N): # N is number of cells
        return (N + 1) ** 2 + 4 * (N + 1)
    ...
    if run:
        ...
        while (diff > tol):
                    ...
                    if j == 0:
                        ...
                        uy_xi[k] = (u[N * (N + 1) + j + i * N] + u[
                            N * (N + 1) + j - 1 + i * N]) * xi[j + i * (N + 1)] / (
                                                2. * h[i])
            t1.append(timeit.default_timer()-titer)
            t2s=timeit.default_timer()
            for i in range(N):
                for j in range(N + 1):
            ...
            t2.append(timeit.default_timer()-t2s)                                                                            
            ...
            # print("Iter %1i: %1.5f s" % (iter, timeit.default_timer()-titer))
            iterlst.append(timeit.default_timer()-titer)
            if ((iter % 100) == 0):
                ...
                print("maxdiv : ", maxdiv)
                print("diff   : ", diff)
                print("Itertime: %1.5f s" % (timeit.default_timer()-startiter))
                print("average iter: %1.5f s" %(np.mean(iterlst)))
                print("t1: %1.5f s" %(np.mean(t1)))
                print("t2: %1.5f s" %(np.mean(t2)))
                startiter=timeit.default_timer()
            iter += 1
    ...
    def plot_streamfunction(N, u, x):
        ...
        phi = sparse.linalg.lsqr(tE10, Ht11 @ u)[0].reshape(N + 1, N + 1)
        phi = phi - phi[0,0]
        ...
        # lvls = [0.115, 0.1175]
        fig, ax = plt.subplots(1, 1, dpi=150)
        cs = ax.contour(X, Y, phi, levels=lvls)
        ax.clabel(cs, inline=1, fontsize=12)
        ax.set_xlabel("x [-]")
        ax.set_ylabel("y [-]")
    ...
    results[N]={'p': p, 'u': u, 'h': h, 'u_pres': u_pres_vort}
    plot_centervals(N, u, p, h,u_pres_vort)
    dataVert=np.loadtxt('vertical_data.txt',skiprows=1)
    pickle.dump(results, open('results.pickle', 'wb'))
    print("Total time for N=%s, is %1.2f s" % (N,timeit.default_timer()-start))